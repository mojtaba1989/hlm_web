import subprocess

def radio_on():
    try:
        result = subprocess.run(
            ["sudo", "nmcli", "radio", "wifi", "on"],
            capture_output=True,
            text=True,
            check=True
        )
        return {"status": "success", "message": result.stdout.strip()}
    except subprocess.CalledProcessError as e:
        return {"status": "error", "message": e.stderr.strip()}
    
def radio_off():
    try:
        result = subprocess.run(
            ["sudo", "nmcli", "radio", "wifi", "off"],
            capture_output=True,
            text=True,
            check=True
        )
        return {"status": "success", "message": result.stdout.strip()}
    except subprocess.CalledProcessError as e:
        return {"status": "error", "message": e.stderr.strip()}


def wifi_connect(ssid, password, dev):
    try:
        subprocess.run(["sudo", "nmcli", "connection", "delete", ssid], 
                       stderr=subprocess.DEVNULL, stdout=subprocess.DEVNULL)
        result = subprocess.run(
            ["sudo", "nmcli", "dev", "wifi", "connect", ssid, "password", password, "ifname", dev],
            capture_output=True,
            text=True,
            check=True
        )
        return {"status": "success", "message": result.stdout.strip()}
    except subprocess.CalledProcessError as e:
        return {"status": "error", "message": e.stderr.strip()}

def wifi_disconnect(dev):
    try:
        result = subprocess.run(
            ["sudo", "nmcli", "dev", "disconnect", dev],
            capture_output=True,
            text=True,
            check=True
        )
        return {"status": "success", "message": result.stdout.strip()}
    except subprocess.CalledProcessError as e:
        return {"status": "error", "message": e.stderr.strip()}

def wifi_status(dev):
    try:
        result = subprocess.run(
            ["nmcli", "-t", "-f", "DEVICE,STATE", "dev"],
            capture_output=True,
            text=True,
            check=True
        )
        for line in result.stdout.strip().split('\n'):
            if line.startswith(dev + ":"):
                state = line.split(":")[1]
                return {"status": "success", "device_state": state}
        return {"status": "error", "message": f"Device {dev} not found"}
    except subprocess.CalledProcessError as e:
        return {"status": "error", "message": e.stderr.strip()}
def list_wifi_networks_nmcli():
    try:
        command = ["nmcli", "-t", "-f", "ssid", "dev", "wifi", "list"]
        output = subprocess.check_output(command, stderr=subprocess.STDOUT, text=True)
        ssids = set()
        for line in output.strip().split('\n'):
            if line:
                if line.startswith("wlan"):
                    ssids.add(line.strip())
        
        return {"status": "success", "ssids": sorted(ssids)}
    except subprocess.CalledProcessError as e:
        return {"status": "error", "message": e.output.strip()}


def list_wifi_devices_nmcli():
    try:
        command = ["nmcli", "-t", "-f", "DEVICE", "dev"]
        output = subprocess.check_output(command, stderr=subprocess.STDOUT, text=True)
        devices = []
        for line in output.strip().split('\n'):
            if line:
                devices.append(line.strip())
        
        return {"status": "success", "devices": devices}
    except subprocess.CalledProcessError as e:
        return {"status": "error", "message": e.output.strip()}
    
def get_ip_address(dev):
    """Gets the IP address of a Wi-Fi device using nmcli."""
    try:
        command = ["nmcli", "-g", "ip4.address", "device", "show", dev]
        output = subprocess.check_output(command, stderr=subprocess.STDOUT, text=True)
        return {"status": "success", "ip_addresses": output.strip()}
    except subprocess.CalledProcessError as e:
        return {"status": "error", "message": e.output.strip()}